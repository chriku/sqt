...
struct sqt {
  ...
  uint32_t count() const { return sqt_impl::sqt_count(data_); }
  uint32_t major() const { return sqt_impl::sqt_major(data_); }
  uint32_t minor(uint32_t i) const { return sqt_impl::sqt_minor(data_, i); }
  void set_major(uint32_t v) {return sqt_impl::sqt_set_major(data_,v);}
  sqt add_minor(uint32_t v)const {return sqt_impl::sqt_add_minor(data_,v);}
